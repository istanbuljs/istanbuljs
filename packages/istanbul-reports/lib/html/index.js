...
function documentTemplate(details, innerTemplate) {
    return `
<!doctype html>
<html lang="en">
    ${htmlHead(details)}
...
function headerTemplate(details) {
    function metricsTemplate({ pct, covered, total }, kind) {
        return `
            <div class='coverage-stat'>
                <span class="coverage-stat__stats">${pct}% <span class="coverage-stat__label">${kind}</span></span>
                <span class='coverage-stat__fraction'>${covered}/${total}<span>
            </div>
        `;
    }
    ...
    function skipTemplate(metrics) {
        ...
        return `
            <div class="coverage-stat coverage-stat--ignored">
                <span class="coverage-stat__stats">${skips.join(
                    ', '
                )} <span class="coverage-stat__label">Ignored</span></span>
            </div>
        `;
    }

    return `
    <header class='header-details header-details--status-${
        details.reportClass
    }'>
        <nav class="header-details__navigation">
            <h1 class="breadcrumbs">${details.pathHtml}</h1>

            <template id="filterTemplate">
                <div class="filter-input">
                    <label for="fileSearch" class="filter-input__label">Filter:</label>
                    <input class="filter-input__input" oninput="onInput()" type="search" id="fileSearch">
                </div>
            </template>
        </nav>

        <div class="header-details__information">
            <div class='header-details__statistics'>
            ...
            <p class="header-details__instructions">
...
function footerTemplate(details) {
    return `
            <div class='footer'>
                <p>Code coverage generated by
                <a href="https://istanbul.js.org/" target="_blank" rel="noopener noreferrer">istanbul</a>
                at ${html.escape(details.datetime)}</p>
            </div>
    `;
}
...
function detailTemplate(data) {
    ...
    const lineLink = num =>
        `<a name='L${num}'></a><a href='#L${num}'><span>${num}</span></a>`;
    ...
    return [
        '<tr>',
        `<td class="line-stat line-stat--count">${lineNumbers
            .map(lineLink)
            .join('\n')}</td>`,
        `<td class="line-stat line-stat--coverage">${data.lineCoverage
            .map(lineCount)
            .join('\n')}</td>`,
...
const summaryTableHeader = [
    ...
    '<thead>',
    '<tr>',
    '   <th data-col="file" data-fmt="html" data-html="true" class="file"><span class="coverage-summary__header-label">File</span</th>',
    '   <th data-col="pic" data-type="number" data-fmt="html" data-html="true" class="pic"></th>',
    '   <th data-col="statements" data-type="number" data-fmt="pct" class="pct"><span class="coverage-summary__header-label">Statements</span</th>',
    '   <th data-col="statements_raw" data-type="number" data-fmt="html" class="abs"></th>',
    '   <th data-col="branches" data-type="number" data-fmt="pct" class="pct"><span class="coverage-summary__header-label">Branches</span</th>',
    '   <th data-col="branches_raw" data-type="number" data-fmt="html" class="abs"></th>',
    '   <th data-col="functions" data-type="number" data-fmt="pct" class="pct"><span class="coverage-summary__header-label">Functions</span</th>',
    '   <th data-col="functions_raw" data-type="number" data-fmt="html" class="abs"></th>',
    '   <th data-col="lines" data-type="number" data-fmt="pct" class="pct"><span class="coverage-summary__header-label">Lines</span</th>',
...
function summaryLineTemplate(details) {
    ...
    const percentGraph = pct => {
        ...
        return `<div class="coverage-bar__progress" style="width: ${percentage}%"></div>`;
    };
    ...
    const summaryType = (type, showGraph = false) => {
        ...
        if (showGraph) {
            result.unshift(
                ...
                `<div class="coverage-bar coverage-bar--${reportClass}">${percentGraph(
                    info.pct
                )}</div>`,
                `</td>`
            );
    ...
    return []
        .concat(
            ...
            `<td class="file ${
                ...
                output
            )}"><span class="file__name">${html.escape(file)}</span></a></td>`,
...
const summaryTableFooter = ['</tbody>', '</table>', '</div>', '</div>'].join(
    '\n'
);
const emptyClasses = {
    statements: 'empty',
...
class HtmlReport extends ReportBase {
    ...
    getBreadcrumbHtml(node) {
        ...
        const linkPath = nodePath.map(ancestor => {
            ...
            const name = ancestor.getRelativeName() || topLevelLabel;

            return `<a href="${target}"><span>${name}</span></a>`;
        });

        linkPath.reverse();

        linkPath.push(`<span>${node.getRelativeName()}</span>`);

        return linkPath.length > 1 ? linkPath.join(' / ') : topLevelLabel;
    }
        ...
        cw.write(
            documentTemplate(
                ...
                <div class="wrapper">
                    ...
                    ${children.reduce((templateStr, child) => {
        ...
        cw.write(
            documentTemplate(
                ...
                <div class="wrapper">
                    ...
                    <div class="table-data table-data--coverage">
                        <pre><table class="coverage">${detailTemplate(
                            annotator(node.getFileCoverage(), context)
                        )}</table></pre>
                    </div>
                </div>
